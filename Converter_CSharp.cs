...
namespace ChromaAPISync
{
    ...
    {
        static bool WriteCSharp(StreamWriter sw)
        {
            try
            {
                string header = HEADER_CSHARP.Replace("__UNITY_INCLUDES__\r\n", string.Empty);
                header = header.Replace("__UNITY_DLL_NAME__\r\n", HEADER_CSHARP_DLL_NAME);
                header = header.Replace("__UNITY_KEY_MAPPING__\r\n", string.Empty);
                header = header.Replace("__UNITY_GET_STREAMING_PATH__\r\n", string.Empty);
                Output(sw, "{0}", header);
